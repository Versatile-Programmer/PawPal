...
const loginController = async (req: Request, res: Response): Promise<void> => {
  try {
    ...
    if(!jwtPayload){
      console.log("Not able to create jwt payload");
    }
    const token = jwt.sign(jwtPayload,process.env.SECRET_KEY as string,{expiresIn:"30d"});
    if (!token) {
...
export { registerController, loginController, userController, checkCredentials };
