...
export const updatePetHandler = async (
  ...
  console.log(
    `[Update Pet by Param: ${petIdStringFromParams}] Received update request. User ID from auth: ${userIdFromAuth}`
  );
  ...
  if (!userIdFromAuth) {
    if (req.file) {
      fs.unlink(req.file.path, (err) => {
        if (err)
          console.error(
            `Error deleting temp file ${req.file?.path} (user not authenticated):`,
            err
          );
      });
    }
    console.log(
      `[Update Pet by Param: ${petIdStringFromParams}] User not authenticated.`
    );
    res.status(401).json({ message: "User not authenticated." });
    return;
  }
  ...
  let petId: bigint;
  let authenticatedUserId: bigint;
  try {
    ...
    if (req.file) {
      fs.unlink(req.file.path, (err) => {
        if (err)
          console.error(
            `Error deleting temp file ${req.file?.path} (invalid petId):`,
            err
          );
      });
    }
    console.log(
      `[Update Pet by Param: ${petIdStringFromParams}] Invalid Pet ID format. Input was: "${petIdStringFromParams}"`
    );
    res
      .status(400)
      .json({
  ...
  try {
    ...
    if (req.file) {
      fs.unlink(req.file.path, (err) => {
        if (err)
          console.error(
            `Error deleting temp file ${req.file?.path} (invalid userId):`,
            err
          );
      });
    }
    console.error(
      `[Update Pet ID: ${petId}] Invalid User ID format for authenticated user: ${userIdFromAuth}`
    );
    res
      .status(500)
  ...
  try {
    ...
    if (existingPet.listedByUserId !== authenticatedUserId) {
      ...
      console.log(
        `[Update Pet ID: ${petId}] User ${authenticatedUserId} not authorized. Pet owned by ${existingPet.listedByUserId}.`
      );
      res
        .status(403)
        .json({ message: "You are not authorized to update this pet." });
    ...
    let newImagePathToStoreInDb: string | null | undefined = undefined;
    ...
    if (uploadedFile) {
      ...
      console.log(
        `[Update Pet ID: ${petId}] New image uploaded: ${newImagePathToStoreInDb}`
      );
    } else if (
      ...
      console.log(
        `[Update Pet ID: ${petId}] No new image uploaded and no removal flag.`
      );
    ...
    const dataForPrismaUpdate: Record<string, any> = { ...validatedUpdateData };
    ...
    console.log(
      `[Update Pet ID: ${petId}] Updating pet in DB with data:`,
      JSON.stringify(dataForPrismaUpdate, null, 2)
    ...
    console.log(
      `[Update Pet ID: ${petId}] Pet updated successfully in DB:`,
      updatedPet
    );

    // 9. Delete old image from filesystem if necessary
    const oldImageShouldBeDeleted =
      (newImagePathToStoreInDb && // A new image path was set (string)
    ...
    if (oldImageShouldBeDeleted && existingPet.imageUrl) {
      ...
      const oldImageAbsolutePath = path.join(
        ...
        "../../public",
        existingPet.imageUrl
      ); // Adjust ../../public as needed

      console.log(
        `[Update Pet ID: ${petId}] Attempting to delete old image: ${oldImageAbsolutePath}`
      );
      fs.unlink(oldImageAbsolutePath, (err) => {
        if (err && err.code !== "ENOENT") {
          console.error(
            `Error deleting old image ${oldImageAbsolutePath}:`,
            err
          );
        } else if (!err) {
          console.log(
            `Successfully deleted old image: ${oldImageAbsolutePath}`
          );
        } else if (err?.code === "ENOENT") {
          console.log(
            `Old image not found (ENOENT), presumed already deleted: ${oldImageAbsolutePath}`
          );
        }
      });
    }
    ...
    const idContextForLog =
      typeof petId === "bigint" ? petId.toString() : petIdStringFromParams;
    console.error(
      `[Update Pet (ID Context: ${idContextForLog})] Unhandled error during update:`,
      error
    );
    res
      .status(500)
