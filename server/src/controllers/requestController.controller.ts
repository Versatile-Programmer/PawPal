...
export const createAdoptionRequest = async (req: Request, res: Response) => {
  try {
    ...
    if (pet.lister?.email) {
      ...
      console.log(
        `Lister notification email job added for ${pet.lister.email}`
      );
    } else {
...
export const approveRequests = async (
  ...
  try {
    const result = await prisma.$transaction(async (tx) => {
      ...
      if (!request) throw new Error("Request not found."); // Custom error type might be better
      ...
      if (request.pet.adoptionStatus !== AdoptionStatus.Available)
        throw new Error(
          `Pet is not Available (${request.pet.adoptionStatus}).`
        );
...
export const rejectRequests = async (
  req: Request,
  res: Response
): Promise<void> => {
  const { id } = req.params;
